...
/**
 * Autogenerated base class representing trip rows
 * in the Travel database.
 *
 * Don't change this file, since it can be overwritten.
 * Instead, change the Travel_Trip.php file.
 *
 * @module Travel
 */
/**
 * Base class representing 'Trip' rows in the 'Travel' database
 * @class Base_Travel_Trip
 * @extends Db_Row
 *
 ...
 * @param {string} [$fields.directions] defaults to null
 * @param {string} [$fields.coordinates] defaults to null
 */
abstract class Base_Travel_Trip extends Db_Row
{
	/**
	 ...
	 * @type string
	 * @default null
	 * json of {userId: {latitude, longitude, heading}}
	 */
	/**
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Travel', 'indexes', 'Trip', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'trip');
		} else {
			$conn = Db::getConnection('Travel');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'trip';
  			if (!$with_db_name)
  				return $table_name;
  			$db = Db::connect('Travel');
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Travel_Trip'))
		);
	}
...
return array (
  ...
  3 => NULL,
	 ...
	 * Method is called before setting the field and verifies if value is string of length within acceptable limit.
	 * Optionally accept numeric value which is converted to string
	 * @method beforeSet_directions
	 * @param {string} $value
	 * @return {array} An array of field name and value
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_directions($value)
	{
		if (!isset($value)) {
			return array('directions', $value);
		}
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('directions', $value);
		...
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".directions");

		return array('directions', $value);			
	}

	/**
	 * Returns the maximum string length that can be assigned to the directions field
	 * @return {integer}
	 */
	function maxSize_directions()
	{
	...
	/**
	 * Returns schema information for directions column
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_directions()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 * Method is called before setting the field and verifies if value is string of length within acceptable limit.
	 * Optionally accept numeric value which is converted to string
	 * @method beforeSet_coordinates
	 * @param {string} $value
	 * @return {array} An array of field name and value
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_coordinates($value)
	{
		if (!isset($value)) {
		...
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".coordinates");
		if (strlen($value) > 65535)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".coordinates");
		return array('coordinates', $value);			
	}

	/**
	 * Returns the maximum string length that can be assigned to the coordinates field
	 * @return {integer}
	 */
	function maxSize_coordinates()
	{
	...
	/**
	 * Returns schema information for coordinates column
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_coordinates()
	{
...
return array (
  ...
  3 => NULL,
	...
	{
		$field_names = array('publisherId', 'streamName', 'directions', 'coordinates');
		$result = $field_names;
